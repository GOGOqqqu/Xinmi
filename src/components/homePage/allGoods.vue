  ...
  <div class="allGoods">
    ...
    <div class="dropdown">
      ...
      <van-dropdown-menu>
        ...
        <van-dropdown-item get-container="body" title="筛选" ref="item" @closed="closedFilter2">
    ...
    <div class="content">
      ...
      <div class="card_list" v-show="commodityList.length !== 0">
        <div class="commodity_card" v-for="(item, index) in commodityList" :key="index"
             @click="clickCard(item.goods_id)">
...
//  计算属性 类似于data概念
  computed: {
    listUrl() {
      return `api/v2/products/list?goods_tag=${this.goodsList.join('_')}&farmer_id=${this.farmerList.join('_')}&web_tag=${this.tagsList.join('_')}&cat_id=0&order_by=${this.value1}&price_range=${this.activeSliderValue.join('_')}&page_size=30&page=1`
    },
...
//  方法集合
  methods: {
    ...
    getFilter() {
      this.$axios({
        method: 'get',
        url: `api/v2/products/list_conditions?cat_id=${this.catId}`,
      }).then(({data}) => {
        console.log('all data', data.results[0])
