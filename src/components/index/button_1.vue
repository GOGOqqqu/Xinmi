<template>
    <div class="button" @click="switchDatePopup">{{ state.birthTime == '' ? (state.now + ' ' + (state.h < 10 ? '0' +
        state.h : state.h) + ':' + (state.m < 10 ? '0' + state.m : state.m) + ':' + (state.s < 10 ? '0' + state.s :
            state.s)) : state.birthTime
}}</div>
</template>
...
const emit = defineEmits<{   //子传父
    (e: 'switchDatePopup'): void
    // (e: '事件名', 传的数据名: 类型): void  //没有返回值
}>()

const switchDatePopup = () => {  //点击事件 测试子传父
    emit('switchDatePopup')
}
...
.button {
    ...
    box-shadow: 4px 8px 19px -3px rgb(0 0 0 / 20%);
